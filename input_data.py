...
class Data:
    ...
    def dry_adiabat(self, temp: float, alt: float) -> List: #Vrací seznam teplot suché adiabaty, která prochází souřadnicemi [temp, alt]
        return [calc.adiab_dry_lift(temp, alt, a) for a in self.Alist]
    
    def mixr_curve(self, dewp: float) -> List: #Vrací seznam teplot křivky směšovacího poměru s počáteční teplotou dewp od zemského povrchu
        curve = [dewp]
        for i in range(1,len(self.Plist)):
            curve.append(calc.adiab_mixr_lift(self.Plist[i], self.Plist[i-1], curve[-1], curve[-1]))
        return curve

    def find_LCL(self) -> tuple:
        for temp, dewp, alt in zip(self.dry_adiabat(self.Tlist[0], self.Alist[0]), self.mixr_curve(self.Hlist[0]), self.Alist):
            if dewp >= temp:
                return (dewp, alt)

    def find_CCL(self) -> tuple:
        for temp, dewp, alt in zip(self.Tlist, self.mixr_curve(self.Hlist[0]), self.Alist):
            if dewp >= temp:
                return (dewp, alt)
    
    def conv_dry_adiabat(self) -> List:
        return self.dry_adiabat(self.find_CCL()[0], self.find_CCL()[1])
    
    def sat_adiabat(self, dewp: float, alt: float) -> List: #Vrací seznam teplot nasycené adiabaty procházející souřadnicemi [temp, alt]
        curve = [dewp]
